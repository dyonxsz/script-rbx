...
local function refreshDropESP()
    ...
    local active = {}
    -- Confirmado pelo spy: MaterialParts ficam em Workspace.Grab.MaterialPart
    local grabF = workspace:FindFirstChild("Grab")
    if not grabF then return end
    local mpF = grabF:FindFirstChild("MaterialPart") or grabF

    for _, obj in pairs(mpF:GetChildren()) do
        if obj:IsA("BasePart") then
            active[obj] = true
            if not dropESP[obj] then
                -- Tenta ler nome do item
                local dropName = "Item"
                for _, ch in pairs(obj:GetChildren()) do
                    if ch:IsA("StringValue") then
                        dropName = ch.Value ~= "" and ch.Value or ch.Name; break
                    end
                ...
                if dropName == "Item" then dropName = mpF.Name end

                -- Cor baseada no drop
                local col = Color3.fromRGB(255,200,60)
                for _, od in ipairs(ORE_DATA) do
                    local low = string.lower(dropName)
                    if low:find(string.lower(od.name),1,true)
                    or (od.drop and low:find(string.lower(od.drop),1,true)) then
                        col = od.color; break
                    end
                end
                ...
                lbl3.TextColor3 = col
                lbl3.Font = Enum.Font.GothamBold; lbl3.TextSize = 12
                lbl3.Text = dropName
